...
(async () => {
  ...
  await p.screenshot({ path: path.join(shots, '1-call.png'), fullPage: false });
  await p.click('#inspTabs button[data-tab="note"]');
  await p.waitForTimeout(200);
  await p.screenshot({ path: path.join(shots, '2-note.png') });
  await p.click('#inspTabs button[data-tab="latency"]');
  await p.waitForTimeout(200);
  await p.screenshot({ path: path.join(shots, '3-lat.png') });

  for (const v of ['approval','review','config','capability','dashboard','eval']) {
    await p.click(`.navbtn[data-view="${v}"]`);
    await p.waitForTimeout(350);
    await p.screenshot({ path: path.join(shots, `${v}.png`), fullPage: true });
  }
